...
import { Check, User, Download, Sparkles } from 'lucide-react';
...
export default function App() {
  ...
  const handleCapture = () => {
    ...
    if (ctx) {
      ...
      }, 1000); // 1s matches the CSS duration below
    }
  };
  ...
  return (
    <div className="relative w-full h-screen bg-slate-50 overflow-hidden flex flex-col md:flex-row font-sans">
      ...
      {/* --- SIDEBAR --- */}
      <div className="relative z-10 w-full md:w-[340px] lg:w-[380px] h-full bg-white/95 backdrop-blur-md border-r border-slate-200 shadow-2xl flex flex-col items-center p-4 overflow-y-auto shrink-0 no-scrollbar">
        ...
        {/* Name Input - Compact Design with Wrapping support */}
        ...
        <div className="relative mb-6 flex justify-center w-full z-20">
            ...
            {/* Printing Animation - Slides UP */}
            {printingPhoto && (
              <div 
                className="absolute top-0 left-1/2 -translate-x-1/2 z-0 transition-all duration-[1000ms] ease-out origin-bottom"
                style={{ 
                   transform: 'translate(-50%, -95%) rotate(-2deg)', // Slide UP out of camera
                   width: '180px', // Smaller scale for the animation
                }}
              >
                 {/* Show the Actual Frame Design developing! */}
        ...
        {/* Frame Selector - 3 Cols - Full Fit Preview */}
        <div className="w-full px-2">
          ...
          <div className="grid grid-cols-3 gap-3">
             {Object.values(FrameStyle).map((style) => (
               ...
               >
                 {/* Preview of the frame design - Centered & Scaled to Fit */}
        ...
        <div className="mt-auto pt-6 text-center text-[9px] text-slate-300 font-mono leading-relaxed">
      ...
      {/* --- CANVAS AREA --- */}
      <div className="flex-1 relative h-full bg-[#f8fafc] overflow-hidden touch-none">
         ...
         <div className="absolute top-6 right-8 z-[60]">
           {photos.length > 0 && (
             <button
               ...
               disabled={isZipping}
               className="bg-zinc-900 hover:bg-zinc-800 text-white shadow-2xl px-6 py-3 rounded-full font-bold flex items-center gap-3 transition-all transform hover:scale-105 active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed border border-zinc-700"
             >
                <Download size={18} className={isZipping ? 'animate-bounce' : ''} />
                <span className="text-sm">{isZipping ? 'Downloading...' : 'Download All'}</span>
         ...
         <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
            {photos.length === 0 && (
                <div className="text-slate-300/60 text-4xl font-script text-center p-12 -rotate-2 select-none border-4 border-dashed border-slate-200 rounded-[3rem]">
      ...
      {/* COUNTDOWN OVERLAY - RED AND HUGE */}
      {countdown !== null && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center pointer-events-none bg-black/20 backdrop-blur-[2px]">
            <div key={countdown} className="text-[20rem] font-bold text-[#b91c1c] drop-shadow-[0_8px_30px_rgba(255,255,255,0.8)] animate-in zoom-in fade-in duration-300 font-sans leading-none">
                {countdown}
            </div>
        </div>
      )}
